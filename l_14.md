# Lesson13 类：动态（继承）多态 *Dynamic Polymorphism*

---
- [Lesson13 类：动态（继承）多态 *Dynamic Polymorphism*](#lesson13-类动态继承多态-dynamic-polymorphism)
  - [1. 多态的基本概念](#1-多态的基本概念)
    - [概念引入](#概念引入)
    - [多态的注意点](#多态的注意点)
  - [2. 多态的应用场景](#2-多态的应用场景)
...
>**vfptr**: virtual function pointer 虚函数（表）指针<br>
指针指向内存中的虚函数表

>**vftable**: virtual function table 虚函数表<br>
表内记录每个虚函数的地址
派生类会继承基类的虚函数表

**多态是如何发生的（虚机制）**
1. 派生类类重写虚函数后，派生类中的虚函数表内部会替换为派生类的虚函数地址
2. 当使用基类指针或引用指向派生类对象时，派生类继承得到的基类虚函数指针被保留
3. 编译器依据虚函数指针动态绑定派生类的虚函数
...
**动态绑定**：即静态类型与动态类型不同，改变指针指向时实际改变的是内存大小（内存切割）而不是类型。
